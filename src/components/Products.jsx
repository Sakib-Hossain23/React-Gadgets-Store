...
function Products({ addToCart }) {
  ...
  const scrollToProductsContainer = () => {
    if (productsContainerRef.current) {
      const offset = -170; // Adjust this value to control scroll position
  ...
  const handlePageChange = (page) => {
    ...
    scrollToProductsContainer(); // Scroll first to avoid delay
    setLoading(true); // Start loading animation

    setTimeout(() => {
      setCurrentPage(page);
      setLoading(false); // Stop loading after 500ms
    }, 500);
  };
  ...
  }, [currentPage]); // Triggered when currentPage changes
